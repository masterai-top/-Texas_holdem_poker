[简体中文](README.md) | [繁體中文](README.zh-TW.md) | [English](README.en.md)

# H5 and Web Online Texas Holdem Poker Platform

> **Proven track record of stable online operation | Club + Alliance + Private Game Support | 10+ Game Variants | Recently taken offline; full source code **
💡 Rapidly deploy your own Texas Hold'em platform
💡 Quickly establish your own poker system


🔥 Online Multiplayer System
🔥 Club + Agent System
🔥 Real-Time Gameplay


👉 🚀 Demo Available
👉 📩 Contact for Full Version
🔥 Multiplayer Poker System
👉 Game Server + Lobby + Club + Tournament


---


## 🧠 Project Overview


This is a multiplayer Texas Hold’em poker game platform.


[![Contact](https://img.shields.io/badge/联系-TG%3A%40xuzongbin001-blue)](https://t.me/xuzongbin001)
[![Platform](https://img.shields.io/badge/客户端-Unity%20iOS%2FAndroid-green)]()
[![Backend](https://img.shields.io/badge/服务端-C%2B%2B-red)]()


---


## ✨ Why Choose This Source Code?


This project is a **real-world online platform that was recently taken offline**;
the code has been battle-tested through years of live operation—this is not merely a demo.
| Core Advantages | Description |
| :--- | :--- |
| 🎮 **8 Game Modes** | Classic Texas Hold'em, Short Deck, Omaha, Pineapple (OFC), MTT, SNG, AOF, etc. |
| 🎙️ **Voice & Video** | Real-time in-game voice chat and video gameplay |
| 👥 **Comprehensive Social Features** | Clubs, Leagues, and Private Games (Friends-only games) |
| 🏆 **Gold Coin Lobby** | Complete in-game economy and store system |
| 🌍 **Multi-language Support** | Supports multiple languages; suitable for global expansion |
| 💰 **Proven Product** | Recently taken offline; stable codebase; ready for immediate launch |


### 🇨🇳 Simplified Chinese
- Complete Texas Hold'em system
- Supports real-time multiplayer matches
- Built-in club and agent systems
- Ready for direct deployment or further customization


### 🇹🇼 Traditional Chinese
- Complete Texas Hold'em system
- Supports real-time multiplayer matches
- Built-in club and agent systems
- Ready for deployment and customization


## ✨ Key Highlights


| Feature | Description |
| :--- | :--- |
| 🎮 **10+ Game Modes** | Classic Texas Hold'em, AOF, Short Deck, Omaha, Pineapple (OFC), MTT, SNG, Texas Cowboy |
| 👥 **Social System** | Clubs, Leagues, Private Games (Friends-only), Voice & Video Chat |
| 🏗️ **Mature Codebase** | Operated for 2 years; recently taken offline; stable and bug-free code |
| 📱 **Cross-platform Support** | Unity Client → iOS + Android |
| ⚙️ **High-performance Backend** | Written in C++; supports high concurrency |


## 🎯 Feature List
✅ Club System ✅ League Mode ✅ Private Games/Friends-only Games
✅ Gold Coin Lobby ✅ Voice Chat ✅ Video Gameplay
✅ MTT/SNG Tournaments ✅ AOF Mode ✅ Short Deck/Omaha
✅ In-game Store ✅ Top-up System ✅ Game Statistics


## ✨ Key Features


-
🧑‍🤝‍🧑 Multiplayer Poker
- 🏆 Club System
- 🧩 Agent System
- ⚡ Real-time Gameplay
- 🌐 Online Server
- 🔧 Customizable (Open to further development)


## 📂 Code Structure
├── gameserver.cpp/h # Main game server logic
├── gameroot.cpp/h # Game room management
├── onclientmessage.cpp/h # Client message handling
├── onroommessage.cpp/h # Room message handling
├── sendclientmessage.cpp/h # Message sending
└── sendroommessage.cpp/h


## 🚀 Tech Stack


- **Client**: Unity (C#) - Supports iOS/Android
- **Server**: C++ - High-performance, stable operation
- **Database**: MySQL + Redis


## 📦 support


- Complete server source code (C++)
- Complete client source code (Unity)
- Database scripts
- Deployment documentation
- Art asset package


## 💰 Get the Complete Source Code


📱 **Telegram: @xuzongbin001**
📧 **Email: masterai918@gmail.com**





## 🎮 Demo


See real gameplay below 👇
![4房间2-9人桌](https://github.com/user-attachments/assets/ad203631-b3a9-4df2-bae9-949afcb0ef8f)
![微信图片_20250515120021](https://github.com/user-attachments/assets/08c0b51e-2c6a-4d93-a5c3-ee8cc99091e5)
![4房间2-6人桌](https://github.com/user-attachments/assets/bcc34562-7f11-4ccc-a151-ead7c190046f)


![微信图片_20250515120009](https://github.com/user-attachments/assets/bfa23dfe-b9a4-4a9e-bb3b-aa7707454094)


![微信图片_20250515115957](https://github.com/user-attachments/assets/71320fed-6e05-4cbc-8948-c501a64b7086)
![微信图片_20250515115943](https://github.com/user-attachments/assets/b7148adf-7ec6-412b-859e-4fefc4c3d7f8)
![微信图片_20250515115934](https://github.com/user-attachments/assets/ab5a362b-7dc8-4485-aede-5b2cf657cf4a)
![微信图片_20250515115921](https://github.com/user-attachments/assets/b0c18c7e-6dcc-4b3e-bfb5-26046e4cdb09)
![微信图片_20250515115912](https://github.com/user-attachments/assets/2713b5f8-7cb0-4d5c-941a-5eb7187bfa9c)
![微信图片_20250515115859](https://github.com/user-attachments/assets/e9751be5-7c2b-40ae-b3f6-dac1fe05b97c)
![微信图片_20250515115847](https://github.com/user-attachments/assets/a06241cc-ef57-41a3-a127-133fa883062d)
![微信图片_20250515115834](https://github.com/user-attachments/assets/e7ac28c3-d625-485b-ade8-30c05e05f51d)
![微信图片_20250515114608](https://github.com/user-attachments/assets/a0d9a3fb-dc4c-47b3-b44f-f1653c0f557a)
![微信图片_20250515114530](https://github.com/user-attachments/assets/9bd75e87-accd-4558-83f5-ec31f5db251c)
---


⭐ If you find this project valuable, please give it a star to show your support~


---


---


## ✅ Add a badge (boosts trust)
```markdown
![Stars](https://img.shields.io/github/stars/masterai-top/Texas-game-source-code?style=social)
![Last Commit](https://img.shields.io/github/last-commit/masterai-top/Texas-game-source-code)
```
